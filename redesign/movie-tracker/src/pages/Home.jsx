    ...
    return (
      <section className="mb-6">
        <div className="flex justify-between items-center mb-3">
          <h2 style={sectionTitleStyle} className="text-lg">{title}</h2>
          <Link
            to={viewAllLink}
            style={viewAllStyle}
        ...
        <div className="flex space-x-4 overflow-x-auto pb-4 scrollbar-hide">
          {movies.map((movie) => (
            ...
            >
              <div style={movieCardStyle} className="aspect-[2/3] overflow-hidden shadow-md transition-all">
                {movie.poster ? (
                  <img
                    src={movie.poster}
                    alt={movie.title}
                  ...
                  <div className="flex items-center justify-center h-full">
                    <span style={{ color: 'var(--bsky-text-muted)' }} className="text-xs">No Image</span>
                  </div>
                )}
              </div>
              <p style={movieTitleStyle} className="mt-2 text-sm truncate group-hover:text-[var(--bsky-text-primary)] transition-colors">
                {movie.title}
              </p>
            </Link>
          ))}
            ...
            {searchResults.length > 0 ? (
              <div className="space-y-3">
                {searchResults.map((movie) => (
                  ...
                  >
                    <div style={searchResultItemStyle} className="flex overflow-hidden hover:border-[var(--bsky-accent-blue)] transition">
                      <div className="w-20 sm:w-24 flex-shrink-0">
                        {movie.poster ? (
                          ...
                          <div className="flex items-center justify-center h-full" style={{ backgroundColor: 'var(--bsky-bg-tertiary)' }}>
                            <span style={{ color: 'var(--bsky-text-muted)' }} className="text-xs">No Image</span>
                          </div>
                        )}
                      </div>
                      <div className="flex-1 p-3 flex flex-col justify-between">
                        <div>
                          <h3 style={{ color: 'var(--bsky-text-primary)' }} className="text-sm sm:text-base font-semibold group-hover:text-[var(--bsky-accent-blue)] transition-colors">
                            {movie.title}
                          </h3>
                          <div className="my-1 flex items-center">
                            <StarRating rating={movie.rating} />
                            <span style={{ color: 'var(--bsky-text-secondary)' }} className="ml-2 text-xs">
                              ({movie.rating}/5)
                            </span>
                          </div>
                        </div>
                        {movie.streamingServices && movie.streamingServices.length > 0 && (
                          <div className="mt-2 flex flex-wrap gap-1.5">
                            ...
                            {movie.streamingServices.length > 3 && (
                              <span style={{ color: 'var(--bsky-text-muted)' }} className="text-xs ml-1">
              ...
              <div style={noResultsStyle} className="text-center py-10 px-4">
                ...
                <p style={{ color: 'var(--bsky-text-muted)' }} className="text-sm mt-1">
          ...
          <div>
            ...
            {recommendations.length === 0 &&
              ...
              watchHistory.length === 0 && (
                <div style={noResultsStyle} className="text-center py-10 px-4 mt-6">
                  <p style={{ color: 'var(--bsky-text-primary)' }}>
