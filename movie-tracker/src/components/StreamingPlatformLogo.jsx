...
function StreamingPlatformLogo({ platform, className = "" }) {
  const logos = {
    ...
    Hulu: {
      url: "https://upload.wikimedia.org/wikipedia/commons/f/f9/Hulu_logo_%282018%29.svg",
    ...
    "Apple TV": {
      url: "https://upload.wikimedia.org/wikipedia/commons/thumb/a/ad/AppleTVLogo.svg/512px-AppleTVLogo.svg.png",
      bgColor: "bg-black",
      textColor: "text-white",
    },
    "Paramount+": {
  ...
  if (!logos[platform]) {
    return (
      <div
        className={`rounded-lg flex items-center justify-center bg-gray-100 ${className}`}
      >
  ...
  return (
    <div
      className={`rounded-lg flex flex-col items-center justify-center p-2 ${bgColor} ${className}`}
    >
      ...
      {className.includes("aspect-square") && (
        <span className={`text-xs font-medium mt-1 ${textColor}`}>
          {platform}
        </span>
      )}
    </div>
